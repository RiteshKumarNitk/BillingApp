...
export default function Footer({ data, config, tenant }: { data: FooterSection['data'], config: WebsiteConfig, tenant: any }) {
  ...
  return (
    <footer className="bg-stone-950 text-stone-400" id="contact">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-16">
          <div className="sm:col-span-2 lg:col-span-1">
            <Link href={`/site/${tenant.id}`} className="flex items-center gap-3 mb-5">
              <Leaf className="w-6 h-6" style={{ color: 'var(--theme-primary)' }} />
              <span className="font-serif text-lg font-bold text-white tracking-tight">{tenant.name}</span>
            </Link>
            <p className="text-sm text-stone-500 leading-relaxed mb-6 max-w-xs">
              {tenant.aboutText || "Pure, organic products for a healthier you. Certified organic, farm-fresh, and sustainably sourced."}
            </p>
            {data.showSocialLinks && (
              <div className="flex gap-3">
                {[
                  ...
                  <a key={i} href={s.href} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-full bg-stone-800 flex items-center justify-center hover:bg-[var(--theme-primary)] hover:text-white transition-all text-stone-400">
          ...
          <div>
            ...
            <ul className="space-y-3">
              {[
                ...
                <li key={link.href}>
                  <Link href={link.href} className="text-sm text-stone-400 hover:text-white transition-colors">{link.label}</Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm text-stone-400">
              {['Shipping Info', 'Returns Policy', 'FAQ', 'Contact'].map(s => (
                <li key={s}>{s}</li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-4 text-sm text-stone-400">
              {[
                { icon: MapPin, text: config.businessInfo?.address || tenant.address || "123 Organic Lane, Green Valley" },
                { icon: Phone, text: config.businessInfo?.phone || tenant.phone || "+91 98765 43210" },
                { icon: Mail, text: config.businessInfo?.email || tenant.email || "hello@organicgrove.com" },
              ].map((item, i) => (
      ...
      <div className="border-t border-stone-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-col sm:flex-row justify-between items-center gap-4 text-sm text-stone-500">
          <p>{copyrightText}</p>
