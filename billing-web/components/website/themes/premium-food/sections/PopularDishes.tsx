...
export default function PopularDishes({ data, config, tenant }: { data: PopularDishesSection['data'], config: WebsiteConfig, tenant: any }) {
  const products = tenant?.products || [];
  
  // Just grab the first 4 products to mock "popular"
          ...
          {displayProducts.map((product: any) => (
            <div key={product.id} className="bg-white rounded-[32px] p-6 shadow-sm border border-gray-100 hover:shadow-xl transition-shadow flex flex-col items-center text-center group cursor-pointer relative overflow-hidden">
              <div className="w-40 h-40 rounded-full overflow-hidden mb-6 shadow-md transform group-hover:scale-105 transition-transform duration-500 bg-gray-50 p-2">
                <img 
                  src={product.imageUrl || 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80'} 
                  alt={product.name}
              ...
              <div className="flex items-center justify-between w-full mt-auto">
                ...
                <button className="px-5 py-2 rounded-full bg-[var(--theme-primary)] text-gray-900 font-bold text-sm hover:scale-105 transition-transform shadow-sm">
                  Add To Cart
                </button>
              </div>
            </div>
