    ...
    <footer className="bg-white border-t border-gray-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-16">
          <div className="sm:col-span-2 lg:col-span-1">
            <Link href={`/site/${siteId}`} className="block mb-5">
              {tenant.logoUrl ? (
                ...
                <span className="font-serif text-lg font-bold tracking-widest text-gray-900 uppercase">{tenant.name}</span>
              )}
            </Link>
            <p className="text-sm text-gray-500 leading-relaxed mb-6 max-w-xs">
              {tenant.aboutText || "Curated collections for the modern lifestyle. Redefining everyday elegance."}
            </p>
            {data.showSocialLinks && (
              <div className="flex gap-3">
                {[
                  ...
                  <a key={i} href={s.href} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-full bg-gray-100 flex items-center justify-center hover:bg-gray-900 hover:text-white transition-all text-gray-500">
          ...
          <div>
            <h4 className="text-sm font-bold text-gray-900 uppercase tracking-widest mb-5">Help</h4>
            <ul className="space-y-3 text-sm text-gray-500">
              {['Customer Service', 'Track Order', 'Returns & Exchanges', 'Shipping'].map(s => (
                <li key={s}>{s}</li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-4 text-sm text-gray-500">
              {[
                { icon: MapPin, text: tenant.address || "123 Fashion Street, City" },
                { icon: Phone, text: tenant.phone || "+1 234 567 8900" },
                { icon: Mail, text: tenant.email || "hello@fashionstore.com" },
              ].map((item, i) => (
