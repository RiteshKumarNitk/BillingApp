...
import { WebsiteConfig, WebsiteSection, HeroSection, FeaturesSection, GallerySection, FooterSection, CategoriesSection, FeaturedProductsSection, NewsletterSection, TestimonialsSection, PromoBannerSection } from '@/lib/website/types';
...
export default function FruitFreshLayout({ config, tenant }: LayoutProps) {
  ...
  const renderSection = (section: WebsiteSection) => {
    switch (section.type) {
      ...
      case 'promo-banner':
        return <PromoBanner key={section.id} data={(section as PromoBannerSection).data} config={config} />;
      case 'testimonials':
  ...
  const primaryColor = config.appearance?.colors?.primary || '#f97316';
  const backgroundColor = config.appearance?.colors?.background || '#fff7ed';
  const textColor = config.appearance?.colors?.text || '#7c2d12';
