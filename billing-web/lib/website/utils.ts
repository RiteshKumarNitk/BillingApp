...
export const themeDefaults: Record<string, (tenant: any) => WebsiteConfig> = {
  'premium-food': (tenant) => ({
    theme: 'premium-food',
    appearance: {
  ...
  'modern-restaurant': (tenant) => ({
    ...
    sections: [
      { id: 's1', type: 'hero', isVisible: true, order: 1, data: { title: `Welcome to ${tenant.name}`, subtitle: tenant.aboutText || 'Experience the best dining in town.' } },
      { id: 's2', type: 'features', isVisible: true, order: 2, data: { title: 'Why Choose Us', subtitle: '', features: [] } },
      { id: 's3', type: 'gallery', isVisible: true, order: 3, data: { title: 'Our Gallery', images: [] } },
      { id: 's4', type: 'footer', isVisible: true, order: 4, data: { copyrightText: `© {year} {tenant}. All rights reserved.`, showSocialLinks: true } }
    ]
  }),
  'fashion-store': (tenant) => ({
    theme: 'fashion-store',
    appearance: {
