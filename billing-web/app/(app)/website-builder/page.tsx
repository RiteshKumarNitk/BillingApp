...
const defaultConfigs: Record<string, (name: string, about?: string) => WebsiteConfig> = {
  'premium-food': (name, about) => ({
    theme: 'premium-food',
    appearance: { colors: { primary: '#EAB308', background: '#FAF9F5', text: '#1F2937' } },
    sections: [
      { id: 's1', type: 'hero', isVisible: true, order: 1, data: { title: 'We Serve The Taste You Love', subtitle: about || '' } },
...
export default async function WebsiteBuilderPage() {
  ...
  if (!config || !config.sections || config.sections.length < 3) {
    config = defaultConfigs['premium-food'](tenant.name, tenant.aboutText || undefined);
  }
