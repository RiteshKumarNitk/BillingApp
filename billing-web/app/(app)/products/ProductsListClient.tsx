...
export default function ProductsListClient({ businessType }: { businessType: string | null }) {
  ...
  const showAvailability = showAvailabilityToggle(businessType);
  ...
  return (
    <div>
      <header className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{areaLabel}</h1>
          <p className="text-sm text-gray-500 mt-1">{total} {productNoun.toLowerCase()}{total !== 1 ? 's' : ''} found</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-white rounded-2xl border border-gray-100 shadow-sm">
          ...
          <p className="text-gray-500 font-medium">No {productNoun.toLowerCase()}s found</p>
          <p className="text-sm text-gray-400 mt-1">Try adjusting your filters or add a new {productNoun.toLowerCase()}.</p>
        </div>
      ) : (
        <>
          <div className="space-y-4">
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {showAvailability && (
                          <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${
                            product.isAvailable !== false ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-500'
                          }`}>
